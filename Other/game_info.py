...
page = requests.get('https://www.mlb.com/starting-lineups/2019-05-16')
...
for player in players:
    ...
    if int(counter / 18) % 2 is 0:
        if game_player_counter < 9:
            games[game_num]['home_team']['lineup'].append(player.contents[0])
        else:
            games[game_num]['away_team']['lineup'].append(player.contents[0])
...
pprint.PrettyPrinter(indent=4).pprint(games[0])
